...
END IF;
    IF EXISTS (
        SELECT 1 FROM business.org_units
        GROUP BY org_account_id HAVING COUNT(*) > 1
    ) THEN
        RAISE EXCEPTION 'Rollback failed: duplicates found in org_account_id. Clean data before rollback.';
END IF;
