...
func TestAcceptInvitation(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func(s *mockCollectionService) {
				s.On("AcceptInvitation", mock.Anything, invitationID, customerID).
					Return(nil).
					Once()
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := newTestRouter()
			r.POST("/collections/invitations/:invitationId/accept", withCustomerID(tt.customerID, h.acceptInvitation))

			w := performRequest(r, http.MethodPost, "/collections/invitations/"+tt.invitationID+"/accept")
