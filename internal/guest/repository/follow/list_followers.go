...
func (r *Repository) ListFollowersEnriched(ctx context.Context, requesterID string, customerID string, cursorTime time.Time, cursorID string, limit int) ([]entity.Follower, error) {
	sql := `
		SELECT 
			cf.id AS follow_id,
...
`
	...
	q := database.Query{
		Name: "follow_repository.ListFollowersEnriched",
		Sql:  sql,
	}
	...
	followers := make([]entity.Follower, 0, len(resultRows))
	for _, row := range resultRows {
		followers = append(followers, row.ToEntity())
	}
