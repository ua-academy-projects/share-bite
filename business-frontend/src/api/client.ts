...
import {
    fetchNotifications as fetchNotificationHistory,
...
const GUEST_CUSTOMER_KEY = 'guest_has_customer';

const setGuestCustomerFlag = (hasCustomer: boolean) => {
    if (hasCustomer) {
        localStorage.setItem(GUEST_CUSTOMER_KEY, '1');
    } else {
...
const isPublicGuestRead = (method: string, url: string) => {
    if (method !== 'get') return false;
    const path = url.split('?')[0];
    if (path === '/posts/' || /^\/posts\/[^/]+$/.test(path)) return true;
    if (/^\/customers\/[^/]+$/.test(path) && path !== '/customers/') return true;
    return false;
};
...
const guestTokenInterceptor = (config: any) => {
    const token = localStorage.getItem('token');
    if (!token || !config.headers) return config;

    const method = (config.method || 'get').toLowerCase();
    const url = config.url || '';
    const hasCustomer = localStorage.getItem(GUEST_CUSTOMER_KEY) === '1';

    if (isPublicGuestRead(method, url) && !hasCustomer) {
        return config;
    }
...
[apiRoot, authApi, guestApi, businessApi].forEach(api => {
    if (api === guestApi) {
        api.interceptors.request.use(guestTokenInterceptor);
    } else {
...
export const apiClient = {
    ...
    updateCustomer: async (data: UpdateCustomerInput) => {
    ...
    // Notifications (notification-service on :4005 or /api/notifications proxy)
    getNotifications: async (limit = 20, offset = 0) => {
        const token = localStorage.getItem('token');
        if (!token) {
            return { items: [] as NotificationItem[], total: 0 };
        }
        try {
            const history = await fetchNotificationHistory(token, limit, offset);
            const items: NotificationItem[] = history.map((n) => ({
                id: n.id,
                type: n.type,
                read: n.isRead,
    ...
    markNotificationAsRead: async (_id: number) => {
        // Mark-read is handled on the notifications page when wired to notificationIDs.
    },
