...
import { Moon, Sun, LogOut, LayoutDashboard, Users, Settings, Shield, Compass } from "lucide-react";

export function Sidebar() {
  const { theme, setTheme } = useTheme();
  const { clearTokens, userPayload } = useAuth();
  ...
  return (
    <aside className="w-64 bg-[#163d32] border-r border-[#2f5e50] p-6 flex flex-col justify-between">
      ...
      <div className="flex flex-col gap-4">
        ...
        <Button
          variant="ghost"
          className="justify-start px-3 text-red-400 hover:text-red-300 hover:bg-red-900/20"
